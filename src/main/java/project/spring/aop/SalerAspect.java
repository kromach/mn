...
@Aspect
public class SalerAspect {

	// 어느 매서드인지 필요하고 어디 부분까지 사용하겠다고 명시  --> 뒤에 Ad가 붙어있는 컨트롤러 매서드까지
		@Pointcut("execution(* project.spring.*.controller.*Controller.*Sa(..))")
		...
		public Object adminAop(ProceedingJoinPoint joinpoint) throws Throwable {
			...
			}else if(session.getAttribute("kind") != null && !session.getAttribute("kind").equals("sales")) {
