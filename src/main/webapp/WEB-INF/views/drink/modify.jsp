	...
	<div class="grid2"> 	
		...
		<div class="detail-item detail-width6">
			...
			<form action="modifyPro" name="dkForm" method="post" enctype="multipart/form-data">
				...
				<input type="hidden" name="dkImg" value="${drinkInfo.dkImg}" />
				<table class="tableCss tbl-lg text-left">
						...
						<td>
							<select id="dkBkind" name="dkBkind" class="sel short required" type="select-one" msg="대분류를" >
								...
								<c:forEach items="${bigCategoryList}" var="bigCategory">
									...
									<option value="${bigCategory.code}" ${selected} >${bigCategory.value}</option>
								</c:forEach>
							</select>
					...
					<tr>
						<th>주류명</th>
						<td>
							<input type="text" name="dkName" class="input-lg required" msg="주류명을" value="${drinkInfo.dkName}" />
						</td>
					</tr>
					<tr>
						<th>대표 이미지</th>
						<td>
							<img class="dkImg" src="${drinkInfo.dkImg}" alt="${drinkInfo.dkName}" onerror="this.src='/resources/img/noImage.jpg'" />
							<%-- <input type="hidden" name="oldDkImage" value="${drinkInfo.dkImg}" /> --%>
							<p class="pad-y10"><input type="file" name="dkimage" /></p>
						</td>
					</tr>
					<tr>
						<th>양조장</th>
						<td>
							<input type="text" name="dkPlace" class="input-lg required" value="${drinkInfo.dkPlace}" />
						</td>
					</tr>
					<tr>
						<th>국가/도시</th>
						<td>
							<input type="text" name="dkCountry" class="input-sm required" placeholder="국가" value="${drinkInfo.dkCountry}" />
							&nbsp;<input type="text" name="dkCity" class="input-sm required" placeholder="도시" value="${drinkInfo.dkCity}" />
						</td>
					</tr>
					<tr>
						<th>알콜 도수</th>
						<td>
							<input type="number" name="dkAlcohol" class="input-xs required" value="${drinkInfo.dkAlcohol}" /> 도
						</td>
					</tr>
					<tr>
						<th>어울리는 안주</th>
						<td>
							<input type="text" name="dkFood" class="input-lg required" value="${drinkInfo.dkFood}" />
						</td>
					</tr>
					<tr>
						<th>평가</th>
					...
					<tr>
						<th>주류 정보</th>
						<td>
							<textarea name="dkContent" id="dkContent" type="textarea" class="required" style="width:98%;">${drinkInfo.dkContent}</textarea>
				...
				<div class="text-center pad-top10">
					<input type="button" class="btn btn-lg btn-blue" value="수정" onclick="insertDrink()" />
...
<script>
	$(function() {
		 ...
		 */
		$("#dkBkind").val("${drinkInfo.dkBkind}").attr("selected", "true").change();
    ...
    function addedSmallCategory(data){
    	...
    	json.forEach(function(item, index) {
    		var selected = ""
    		if ("${drinkInfo.dkSkind}" === item.code) {
    			selected = " selected";
    		}
    		var optionStr = "<option value='"+ item.code + "'" + selected +">"+item.value+"</option>"
    ...
    function getItemValues(bigCategory) {
    	$.ajax({
			...
			success : function(data){
				...
				$("#star_grade a.item1[value='${drinkInfo.dkItem1}']").click();
				$("#star_grade a.item2[value='${drinkInfo.dkItem2}']").click();
				$("#star_grade a.item3[value='${drinkInfo.dkItem3}']").click();
				$("#star_grade a.item4[value='${drinkInfo.dkItem4}']").click();
				$("#star_grade a.item5[value='${drinkInfo.dkItem5}']").click();
			},
			error : function() {
				alert("error");
			}
